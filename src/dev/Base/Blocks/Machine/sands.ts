  ...
  [
    {
      ...
      texture: [
        ["blockSoulMachineBottom", 0],
        ["blockSoulMachineTop", 0],
        ["blockSoulMachineSide", 0],
        ["sliceAndSpliceFront", 0],
        ["blockSoulMachineSide", 0],
        ["blockSoulMachineSide", 0],
      ],
      inCreative: true,
...
TileRenderer.setHandAndUiModel(BlockID.sliceAndSplice, 0, [
  ["blockSoulMachineBottom", 0],
  ["blockSoulMachineTop", 0],
  ["blockSoulMachineSide", 0],
  ["sliceAndSpliceFront", 0],
...
TileRenderer.setStandardModelWithRotation(BlockID.sliceAndSplice, 2, [
  ["blockSoulMachineBottom", 0],
  ["blockSoulMachineTop", 0],
  ["blockSoulMachineSide", 0],
  ["sliceAndSpliceFront", 0],
...
TileRenderer.registerModelWithRotation(BlockID.sliceAndSplice, 2, [
  ["blockSoulMachineBottom", 0],
  ["blockSoulMachineTop", 0],
  ["blockSoulMachineSide", 0],
  ["sliceAndSpliceFrontOn", 0],
  ["blockSoulMachineSide", 0],
  ["blockSoulMachineSide", 0],
]);
...
Callback.addCallback("PreLoaded", function () {
  ...
  Recipes.addShaped(
    { id: BlockID.sliceAndSplice, count: 1, data: 0 },
    ["shs", "amc", "sss"],
...
namespace Machine {
  export class SliceAndSplice extends BasicMachine {
    ...
    acceptType = ["cleancut", "buffer", "intake"];
    initCapacitor(): void {
      const capacitors = CapacitorData.useCapacitor(this);
      this.energyConsume = capacitors.getValue(CapacitorKey.SLICE_POWER_USE);
      this.maxIntake = capacitors.getValue(CapacitorKey.SLICE_POWER_INTAKE);
      this.energyStorage = this.updateEnergyStorage(capacitors.getValue(CapacitorKey.SLICE_POWER_BUFFER));
    }
    ...
    setupContainer(): void {
      StorageInterface.setGlobalValidatePolicy(this.container, (name, id, amount, data) => {
        ...
        if (name == "slotInput")
          return SliceAndSpliceRecipe.getRecipeWithItem({
            id: id,
            data: data,
            count: amount,
          });
    ...
    getAxe(): boolean {
      ...
      if (accept_axe[slotAxe.id]) return !!accept_axe[slotAxe.id];
    }

    getShears(): boolean {
      let slotShears = this.container.getSlot("slotShears");
      if (accept_shear[slotShears.id]) return !!accept_shear[slotShears.id];
    }
    ...
    run(): void {
      ...
      if (
        ...
        this.getAxe() &&
        this.getShears() &&
        ((output.id == recipe.result.id && output.count < 64 + recipe.result.count && output.data == recipe.result.data) || output.id == 0)
      ) {
