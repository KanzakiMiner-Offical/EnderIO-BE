namespace EnderCore {
  ...
  export function registerDust(types: string[]) {
    for (let i in types) {
      ...
      ItemRegistry.createItem("dust" + types[i], {
        name: name,
        icon: "dust" + types[i],
        stack: 64,
      });
  ...
  export function createResourceItem(id, name) {
    ...
    ItemRegistry.createItem(nug_id, {
      name: nugget_name,
      icon: nug_id,
      stack: 64,
    });
    ...
    Callback.addCallback("PreLoaded", function () {
      Recipes.addShaped({ id: ItemID[id], count: 1, data: 0 }, ["bbb", "bbb", "bbb"], ["b", ItemID[nug_id], 0]);
  ...
  export function createResourceBlock(id, name) {
    ...
    Callback.addCallback("PreLoaded", function () {
      Recipes.addShaped({ id: BlockID[block_id], count: 1, data: 0 }, ["bbb", "bbb", "bbb"], ["b", ItemID[id], 0]);
