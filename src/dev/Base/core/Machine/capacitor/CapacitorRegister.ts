...
class DefaultCapacitor extends ItemCommon implements ItemBehavior, ICapacitorData {
  type = "capacitor";
  level: number;
  ...
  onNameOverride(item: ItemInstance, name: string): string {
    return name + "\nBase level: " + this.getLevel(item);
  }

  getLevel(item: ItemInstance, machine?: TileEntity): number {
    return item.extra ? item.extra.getFloat("level") : this.level;
  }
}

class CapacitorLoot extends ItemCommon implements ItemBehavior, ICapacitorData {
  type: "loot_capacitor";
  canBeDMG: boolean = false;
  ...
  getBaseLevel(item: ItemInstance): number {
    // For All Machine, except Machine Level
