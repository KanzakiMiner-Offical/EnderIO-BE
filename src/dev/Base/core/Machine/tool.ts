  ...
  Callback.addCallback("DestroyBlockStart", function (coords: Callback.ItemUseCoordinates, block: Tile) {
    if (MachineRegistry.isMachine(block.id)) {
      ...
      if (EnderTool.isUseableWrench(item, 10)) {
        Network.sendToServer("EnderCore.demontageMachine", {
          x: coords.x,
          y: coords.y,
          z: coords.z,
        });
...
namespace ModelHelper {
  export function renderModel(model: string, import_params: com.zhekasmirnov.innercore.api.NativeRenderMesh.ImportParams): RenderMesh {
    ...
    mesh.importFromFile(__dir__ + "resources/assets/models/" + model + ".obj", "obj", import_params || null);
    return mesh;
  }
  ...
  export function setInventoryModel(
    item: ItemStack,
    model_name: string,
    texture: string,
  ...
  export function registerModelWithRotation(block: number, dir: string, texture: [string, number], importParams?: com.zhekasmirnov.innercore.api.NativeRenderMesh.ImportParams) {
    ...
    // Block.registerPlaceFunction(block, (coords: Callback.ItemUseCoordinates, item: ItemInstance, block: Tile, player: number, region: BlockSource): Vector | void => {
    //   var place = World.canTileBeReplaced(block.id, block.data) ? coords : coords.relative;
    //   var rotation = TileRenderer.getBlockRotation(player);
    //   region.setBlock(place.x, place.y, place.z, item.id, rotation);
    //   return place;
    // });

    TileRenderer.setRotationFunction(block);
  }
}
