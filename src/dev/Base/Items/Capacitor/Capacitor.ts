...
class CapacitorModule extends ItemCommon implements ICapacitor {
  type: string;
...
class CapacitorCraft extends CapacitorModule implements ItemBehavior {
  type = "capacitor";
  bonus: number;
  consume: number;
  storage: number;
  ...
  */
  ...
  onNameOverride(item: ItemInstance, name: string): string {
    return (
      name +
      "§7\nGenerator Bonus: x" +
      this.getBonusGenerator() +
      "\nEnergy Consume Multiplier: x" +
      this.getEnergyConsumeMultiplier() +
      "\nExtra Energy Storage: +" +
      this.getExtraEnergyStorage() +
      "\nRange: +" +
      this.getRange()
    );
  }
