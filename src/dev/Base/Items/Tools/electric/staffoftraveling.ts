class ItemTravelStaff extends ItemElectric implements ItemBehavior {
  energyPerUse = 3000;

  constructor() {
    super("itemTravelStaff", "Staff of Traveling", 250000, 1000, 1);
  }
  ...
  onNoTargetUse(item: ItemStack, player: number): void {
    ...
    if (client && EnderTool.useElectricItem(item, this.energyPerUse, player)) {
      ...
      client.sendMessage(
        "Player at: X: " +
          Math.round(Entity.getPosition(player).x) +
          " Y: " +
          Math.round(Entity.getPosition(player).y) +
          " Z: " +
          Math.round(Entity.getPosition(player).z),
      );
    }
  }
}
